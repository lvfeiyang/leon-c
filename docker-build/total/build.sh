...
function setup_fmt() {
  ...
  if [ ! -d "$FMT_DIR" ] ; then
    ...
    git clone -b 8.0.0 https://github.com/fmtlib/fmt.git  "$FMT_DIR"
  fi
  cd "$FMT_DIR"
  git fetch --tags
  git checkout 6.2.1
...
function setup_googletest() {
  ...
  if [ ! -d "$GTEST_DIR" ] ; then
    ...
    git clone -b release-1.11.0 https://github.com/google/googletest.git  "$GTEST_DIR"
  fi
  cd "$GTEST_DIR"
  git fetch --tags
...
function setup_zstd() {
  ...
  if [ ! -d "$ZSTD_DIR" ] ; then
    ...
    git clone -b v1.5.0 https://github.com/facebook/zstd.git "$ZSTD_DIR"
  fi
...
function setup_folly() {
  ...
  if [ ! -d "$FOLLY_DIR" ] ; then
    ...
    git clone -b v2021.06.14.00 https://github.com/facebook/folly.git "$FOLLY_DIR"
  fi
...
function setup_fizz() {
  ...
  if [ ! -d "$FIZZ_DIR" ] ; then
    ...
    git clone -b v2021.06.14.00 https://github.com/facebookincubator/fizz "$FIZZ_DIR"
  fi
...
function setup_wangle() {
  ...
  if [ ! -d "$WANGLE_DIR" ] ; then
    ...
    git clone -b v2021.06.14.00 https://github.com/facebook/wangle "$WANGLE_DIR"
  fi
...
function setup_mvfst() {
  ...
  pushd "$MVFST_DIR"
  git checkout 25d6b6d2a26d85fffbbd9be8533afa951a03fc58
  popd
